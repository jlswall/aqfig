...
##
##   2013-10-28 (JLS): To match the arguments in "..." to par() or to
##      plot(), I had been using "graphics:::.Pars".  This is no longer
##      acceptable on CRAN.  So, I have switched to using
##      formals(plot.default) to figure out which of the arguments in
##      "..." go with plot(), and assume the others go with par().
## ###########################################################
aspect.ratio.plot <- function(x, y, file, dev="pdf", type="p", xlim, ylim,
  ...
  ## Look at the arguments in the "..." pairlist.  If these are not
  ## typical arguments to plot.default(), we pass them to
  ## init.fig.dimen(), which uses them in a call to par().  We used to
  ## get the list of par() arguments from graphics:::.Pars, as
  ## explained in the "Details" section of the par() help page, but
  ## this is no longer allowed for CRAN packages.
  possible.plot.args <- names(formals(plot.default))
  extra.args.passed.in <- match.call(expand.dots= FALSE)$...
  extra.args.to.plot <- extra.args.passed.in[names(extra.args.passed.in) %in% possible.plot.args]
  ## Identify arguments that are not specific to plot.default(); they are
  ## later passed to init.fig.dimen(), and from there to par().
  extra.args.to.par <- extra.args.passed.in[!(names(extra.args.passed.in) %in% possible.plot.args)]
